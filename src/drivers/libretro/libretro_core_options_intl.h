...
struct retro_core_option_v2_category option_cats_chs[] = {
   ...
   },
   {
      "hacks",
      "模拟器调整",
      "更改处理器超频和模拟精度设置，这些会影响低级模拟（LLE）的性能和兼容性。"
   },
   {
      "dip_switch",
      "DIP开关",
...
struct retro_core_option_v2_definition option_defs_chs[] = {
   ...
   },
   {
      ...
      "金手指卡（需要重启）",
      NULL,
      "启用金手指卡的模拟，可以在游戏启动时输入金手指码。金手指卡ROM文件‘gamegenie.nes’必须放在前端的系统目录下。此选项对磁碟机和街机游戏无效。",
      NULL,
      "system",
      {
         { "disabled", NULL },
...
#endif /* overscan options */
   {
      ...
      "调色板",
      NULL,
      "可以选择预设调色板、从文件读取的自定义64x3调色板或者原始格式（需要使用 nes-解码器 着色器）",
      NULL,
      "video",
      {
         { "default",              "缺省" },
...
#ifdef HAVE_NTSC_FILTER
   {
      ...
      "NTSC滤镜",
      NULL,
      "Blargg的NTSC滤镜，用于重现射频信号、复合视频、S-Video和RGB信号的效果。",
      NULL,
      "video",
      {
         { "disabled",   NULL },
   ...
   },
   {
      ...
      "音频RF滤镜",
      NULL,
      "使用低通滤波器来模拟使用射频调制器连接到电视时的‘静音’效果。",
      NULL,
      "audio",
      {
         { "disabled", NULL },
   ...
   },
   {
      ...
      "立体声效果",
      NULL,
      "当对NES单声道音频输出进行上混时，通过延迟右声道来实现伪立体声效果。",
      NULL,
      "audio",
      {
         { "disabled",    NULL },
   ...
   },
   {
      ...
      "光枪模式",
      NULL,
      "选择用于光枪游戏的设备。‘准心跟踪光枪’常用于街机，相当于一种变形的模拟摇杆，游戏检测光枪转动方向来判断是否瞄准屏幕上的准心，而非使用光学检测方法；‘顺序目标检测光枪’即传统FC/NES游戏使用的光枪，使用顺序检测屏幕像素块变化来判断瞄准位置，只能用于CRT显示器。",
      NULL,
      "input",
      {
         { "clightgun",   "准心跟踪光枪" },
   ...
   },
   {
      ...
      "反转光枪传感器信号",
      NULL,
      "使用光枪时反转传感器逻辑（只适用于顺序目标检测光枪）。禁用此项以和原始FC/NES硬件行为一致。",
      NULL,
      "input",
      {
         { "enabled",  NULL },
   ...
   },
   {
      ...
      "允许同时按下相反方向",
      NULL,
      "允许同时按下上下或者左右键，这在有些游戏中可能会产生不同效果。",
      NULL,
      "input",
      {
         { "disabled", NULL },
   ...
   },
   {
      ...
      "解除角色数量限制",
      NULL,
      "移除每条扫描线8个角色数的硬件限制。这可以减少角色闪烁，但是对于使用这个限制来实现某些特效的游戏，可能会导致问题。",
      NULL,
      "hacks",
      {
         { "disabled", NULL },
   ...
   },
   {
      ...
      "超频",
      NULL,
      "启用或者禁用超频，在有些游戏中可以减小拖慢现象。‘后渲染’方法对所有游戏来说兼容性更好；‘垂直消隐’对于有些游戏更加有效（例如《魂斗罗力量》）。",
      NULL,
      "hacks",
      {
         { "disabled",      NULL },
         { "2x-Postrender", "2x-后渲染" },
         { "2x-VBlank",     "2x-垂直消隐" },
         { NULL, NULL },
      },
      "disabled",
   },
   {
      ...
      "开机内存填充（需要重启）",
      NULL,
      "开机时的内存值。有些游戏依赖初始内存值实现某些功能，比如随机数生成。",
      NULL,
      "hacks",
      {
         { "fill $ff", "$FF" },
